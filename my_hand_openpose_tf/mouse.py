...
steps=4 # 1 for more accuracy but less speed , 4 for more post processing speed but less accuracy
...
def process_image(model, image_orig, scale_mul=1, peaks_th=0.1, sigma=3, mode='heatmap'):
    ...
    return image_out, took,center_x,center_y
...
while 1:
    ...
    start = clock()
    image_out, inference_took,X,Y = process_image(model, image,mode='none')
    stop = clock()
    took = stop-start
